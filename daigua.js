...
================Loon==============
[Script]
cron "1,2,3 0 * * *" script-path=daigua.js, tag=呆瓜助力池s
 */
var version='1.0.5';
var fs = require("fs");
console.log(":<<EOF");
...
addSource("jd_get_share_code.js", function () {
    ...
    if (process.env.DAIGUA_READ_SHARE_CODE) {
        getShareCodeRes = getShareCodeFrom_get_share_code_js_log_ByDir(
            process.env.DAIGUA_READ_SHARE_CODE);
    } else {
...
function uploadAndGetShareCodes(data) {
    if (process.env.DAIGUA_HOST && process.env.DAIGUA_TOKEN) {
        var host = process.env.DAIGUA_HOST;
        var token = process.env.DAIGUA_TOKEN;
        var askPtPin = process.env.DAIGUA_FOR;
        var closeSelf = process.env.DAIGUA_CLOSE_SELF;
        var random = process.env.DAIGUA_RANDOM;
        var ops = {
